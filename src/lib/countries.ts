// ISO 3166-1 alpha-2 codes with international dialing codes.
// Names are localized at runtime with Intl.DisplayNames.
export const COUNTRIES: readonly [code: string, dial: string][] = [
...
export function countryOptions(locale: string): CountryOption[] {
  ...
  try {
    names = new Intl.DisplayNames([locale === 'id' ? 'id' : 'en'], { type: 'region' });
  } catch {
    names = null;
  }
  return COUNTRIES.map(([code, dial]) => ({ code, dial, name: names?.of(code) ?? code })).sort((a, b) =>
    a.name.localeCompare(b.name, locale),
  );
}

export function dialCodeOptions() {
  return Array.from(new Set(COUNTRIES.map(([, dial]) => dial)))
